...
module.exports = {
	...
	async execute(message, args, bot) {
        ...
        if (!message.member.voice.channel?.members.has(bot.user.id)) return bot.utils.error("Бот должен быть в одном канале с вами!", this, message, bot)
		...
		return message.channel.send({
			embeds: [
				new MessageEmbed()
					.setDescription(`Громкость трека изменена на **${volume}%**`),
			],
		});
	},
};
