module.exports = {
	...
	async execute(message, args, bot) {
		...
		if (member.user.bot) return bot.utils.error("Это бот!", message);
		if (member.id === message.author.id) bot.utils.error("Как вы поженитесь на себе?", message);
		...
		if (DBuser.marry) return bot.utils.error("Он(-а) уже состоит в браке!", message);
		if (DBauthor.marry) return bot.utils.error("Вы уже состоите в браке!", message);
