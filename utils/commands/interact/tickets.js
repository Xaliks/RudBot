...
module.exports = async (interaction) => {
	const { message, user, member } = interaction;
			...
			channel.send({
				content: `Тикет <@${user.id}>.\n\n<@748468559897952276> и <@678250906407403531> скоро подойдут.`,
				components: [
					{
						type: 1,
						components: [new MessageButton().setLabel("ЗАКРЫТЬ ТИКЕТ").setStyle("DANGER").setCustomId("ticket_delete")],
					},
				],
			});
			return interaction.reply({ content: `Вы открыли тикет! <#${channel.id}>`, ephemeral: true });
}